...
export default function AdminApprovals() {
  ...
  const fetchUsers = async () => {
    ...
    setError('');
    try {
      const res = await fetch('https://backend-omega-teal.vercel.app/api/auth/all', {
  ...
  const saveUser = async (id) => {
    try {
      ...
      const res = await fetch(`https://backend-omega-teal.vercel.app/api/auth/${id}`, {
        method: 'PUT',
        headers: {
  ...
  const approve = async (id) => {
    try {
      ...
      const res = await fetch(`https://backend-omega-teal.vercel.app/api/auth/approve/${id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
  ...
  const saveRole = async (id, role) => {
    try {
      setBusy(prev => ({ ...prev, [id]: true }));
      const res = await fetch(`https://backend-omega-teal.vercel.app/api/auth/role/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
